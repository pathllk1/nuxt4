...
const onStockSelect = (stock: any) => {
  state.cart.push({
    ...
    qty: 1,
    uom: stock.uom || 'PCS',
    rate: stock.selling_price || stock.rate || 0,
...
const saveInvoice = async () => {
  ...
  try {
    const res = await createSalesBill(payload);
    if (res?.success) {
      router.push('/accounting/bills');
    }
  } catch (err: any) {
    alert(err.data?.message || err.message || 'Failed to save sales invoice');
  }
};
  ...
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm">
      <div>
        <span class="px-3 py-1 bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-full text-xs font-bold border border-emerald-500/30">
          Sales & Outward Tax Invoice
        </span>
        <h1 class="text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white mt-1">Create Sales Invoice</h1>
      </div>

      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2 text-xs font-bold">
          <span class="text-slate-400">Bill No:</span>
          <span class="font-mono text-indigo-600 dark:text-indigo-400">#{{ state.meta.billNo || 'AUTO' }}</span>
        </div>
        <button type="button" @click="saveInvoice" :disabled="loading" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-2xl transition-colors shadow-sm disabled:opacity-50">
          {{ loading ? 'Saving...' : 'Save & Post Invoice' }}
        </button>
      </div>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-1 space-y-4">
        <PartyManager
          ...
          empty-subtitle="Select customer record"
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl p-5 border border-slate-200 dark:border-zinc-800 shadow-sm space-y-3 text-xs">
          ...
          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase">Vehicle No</label>
            <input type="text" v-model="state.meta.vehicleNo" placeholder="KA-01-AB-1234" class="w-full mt-1 px-3 py-1.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg font-bold" />
          </div>
          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase">Dispatch Through</label>
            <input type="text" v-model="state.meta.dispatchThrough" placeholder="Transport / Courier" class="w-full mt-1 px-3 py-1.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg font-bold" />
          </div>
        </div>
      </div>
    ...
    <div v-if="showPartyModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div class="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
        <div class="flex justify-between items-center">
          <h3 class="text-base font-bold text-slate-900 dark:text-white">Choose Customer Party</h3>
          <button @click="showPartyModal = false" class="text-slate-400 hover:text-slate-600">✕</button>
        </div>
        <input type="text" v-model="partySearchQuery" placeholder="Search party name or GSTIN..." class="w-full px-3 py-2 bg-slate-50 dark:bg-zinc-800 border rounded-xl font-bold text-xs" />
