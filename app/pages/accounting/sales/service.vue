  ...
  <div class="invoice-page" ref="pageContainerRef">
    <header class="page-header">
      <div class="title-block">
        <p>{{ currentFirmName }}</p>
        <h1>Service Sales Invoice</h1>
      </div>

      <div class="header-fields">
        <label>
          <span>Bill No</span>
          <input :value="meta.billNo || 'ASI/AUTO'" type="text" readonly class="readonly-input font-mono" />
        </label>
        <label>
      ...
      <div class="header-actions">
        ...
        <button class="primary-btn" type="button" :disabled="saving || !canSave" @click="handleSave" title="Save Invoice (F8)">
    ...
    <UModal v-model:open="showSuccessModal" title="Service Invoice Saved Successfully">
      <template #body>
        <div 
        ...
        >
          ...
          <div>
            <h3 class="text-lg font-black text-gray-900 dark:text-white">Service Invoice #{{ createdBill?.bno }} Saved</h3>
    ...
    <UModal v-model:open="showHelpModal" title="Service Invoice Keyboard Cheatsheet">
      <template #body>
        <div class="p-4 space-y-4 text-xs">
          <div class="grid grid-cols-2 gap-3">
            <div class="p-3 bg-slate-50 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700">
              ...
              <p class="text-[10px] text-slate-400">Search & add income ledger account</p>
            </div>
            <div class="p-3 bg-slate-50 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700">
            ...
            <div class="p-3 bg-slate-50 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700">
              ...
              <p class="text-[10px] text-slate-400">Add freight, delivery, handling</p>
            </div>
            <div class="p-3 bg-slate-50 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700">
            ...
            <div class="p-3 bg-slate-50 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700">
              ...
              <p class="font-bold text-slate-800 dark:text-white mt-1">New Party</p>
              <p class="text-[10px] text-slate-400">Quick create new party record</p>
            </div>
          </div>
      ...
      <div class="party-drawer" role="dialog" aria-modal="true">
        <header class="drawer-head">
          ...
          <div class="flex items-center gap-2">
            <button type="button" class="create-party-btn text-xs font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-2 py-1 rounded cursor-pointer" @click="openCreatePartyFromDrawer">
              + New Party (Insert)
            </button>
...
const api = useApi();
const { cart, addItem, removeItem, updateItem, clearCart, computeTotals, saving, saveInvoice, saveError } = useAccountingInvoiceState('SALES');
...
function advanceToPartyOrReference() {
  if (!selectedParty.value) {
    // If party not selected yet, automatically pop the party drawer!
    openPartyDrawer();
  } else {
    referenceInputRef.value?.focus();
    referenceInputRef.value?.select();
...
function onNarrationKeydown(e: KeyboardEvent) {
  if (e.key === 'Enter') {
    if (e.shiftKey) return; // Allow Shift+Enter for new line in textarea
    e.preventDefault();
...
const computedTotals = computed(() => {
  return computeTotals(meta.value.billType, meta.value.reverseCharge);
});

const canSave = computed(() => {
  return selectedParty.value && cart.value.length > 0 && cart.value.some(item => (parseFloat(String(item.amount)) || 0) > 0);
});
...
async function fetchNextBillNo() {
  try {
    const res = await api.get('/accounting/bills/get-next-number?type=ACCOUNTING_SALES');
...
async function handleSave() {
  ...
  try {
    ...
    alert(saveError.value || 'Failed to save invoice');
  }
}
...
async function downloadCreatedPDF() {
  ...
  try {
    await api.download(`/accounting/bills/${createdBill.value._id}/pdf`, `Invoice_${createdBill.value.bno || 'ASI'}.pdf`);
  } catch (err) {
...
async function downloadCreatedExcel() {
  if (!createdBill.value?._id) return;
  try {
    await api.download(`/accounting/bills/${createdBill.value._id}/excel`, `Invoice_${createdBill.value.bno || 'ASI'}.xlsx`);
  } catch (err) {
...
// ─── Party Drawer Keyboard Navigation ───
...
function handleGlobalKeydown(e: KeyboardEvent) {
  // If ANY modal or drawer is active, let the modal handle its keys
  if (
...
.invoice-page {
  ...
  background: linear-gradient(160deg, #f0f6ff 0%, #f8faff 40%, #eef4f9 100%);
}

:root.dark .invoice-page {
  color: #e4e4e7;
  background: linear-gradient(160deg, #09090b 0%, #141416 40%, #09090b 100%);
}
...
.title-block p { font-size: 9px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.2em; color: #10b981; }
...
:root.dark .header-fields input,
:root.dark .header-fields select {
  background: #18181b;
  border-color: #3f3f46;
  color: #e4e4e7;
}
...
.header-fields select:focus { border-color: #3b82f6; }
...
.primary-btn span { font-size: 9px; opacity: 0.7; font-family: monospace; }
...
:root.dark .field-grid textarea { background: #18181b; border-color: #3f3f46; color: #e4e4e7; }
.field-grid input:focus,
.field-grid textarea:focus { border-color: #3b82f6; }
...
.totals-row { display: flex; justify-content: space-between; align-items: center; font-size: 11px; font-weight: 600; color: #475569; }
...
.totals-row.grand { padding-top: 8px; border-top: 2px solid #e2e8f0; font-size: 14px; font-weight: 900; color: #059669; }
:root.dark .totals-row.grand { border-top-color: #3f3f46; color: #34d399; }
.totals-row.grand .amount { font-size: 16px; color: #059669; }
:root.dark .totals-row.grand .amount { color: #34d399; }
