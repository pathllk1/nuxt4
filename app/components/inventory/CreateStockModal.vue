<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" @keydown="handleModalKeydown">
    <div ref="modalContainerRef" class="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-slate-200 dark:border-zinc-800 animate-scale-in flex flex-col">
       <header class="bg-gradient-to-r from-indigo-600 to-blue-600 px-8 py-5 text-white flex justify-between items-center shrink-0">
          <div>
             <h2 class="text-xl font-black uppercase tracking-tighter">Create New Stock Item</h2>
             <p class="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-0.5">Enter: Next Field • Insert: Add Batch • F8: Save Item • ESC: Close</p>
          </div>
       ...
       <div class="overflow-y-auto p-6 flex-1 custom-scrollbar">
         <form @submit.prevent="saveStock" id="stock-form" class="space-y-6">
            <!-- Basic Info -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div class="md:col-span-3">
                  ...
                  <input 
                    ...
                    class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-zinc-800 border-2 border-transparent rounded-xl focus:bg-white dark:focus:bg-zinc-900 focus:border-indigo-500 outline-none transition-all font-bold text-sm text-gray-900 dark:text-white" 
               ...
               <div>
                  ...
                  <input 
                    ...
                    @keydown.enter.prevent="onInputEnter($event)"
                    @keydown.backspace="onInputBackspace($event)"
                    class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-zinc-800 border-2 border-transparent rounded-xl focus:bg-white dark:focus:bg-zinc-900 focus:border-indigo-500 outline-none transition-all font-mono font-bold uppercase text-sm text-gray-900 dark:text-white" 
            ...
            <div class="space-y-3 pt-2">
               <div class="flex items-center justify-between">
                  <h3 class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider">Opening Batches</h3>
                  ...
                  <table class="w-full text-left text-xs">
                     <thead>
                        <tr class="bg-slate-100 dark:bg-zinc-800 text-[10px] font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest">
                           ...
                           <th class="px-3 py-2.5 w-24">UOM *</th>
                        ...
                        <tr v-for="(batch, index) in form.batches" :key="index" class="bg-white dark:bg-zinc-900" :data-batch-row="index">
                           <td class="px-2 py-1.5">
                              <input 
                                ...
                                @keydown.enter.prevent="onInputEnter($event)"
                                @keydown.backspace="onInputBackspace($event)"
                                class="w-full px-2 py-1.5 border-b border-transparent focus:border-indigo-500 outline-none transition-all font-bold text-gray-900 dark:text-white bg-transparent" 
               ...
               <p v-if="form.batches.length === 0" class="text-center py-6 text-slate-400 text-xs font-bold uppercase tracking-widest border-2 border-dashed border-slate-100 rounded-2xl">At least one batch is required</p>
            </div>
         </form>
       </div>
       ...
       <footer class="p-6 border-t border-slate-100 dark:border-zinc-800 flex justify-between items-center shrink-0 bg-slate-50 dark:bg-zinc-900">
          <div class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
             Shortcut: Press <kbd class="px-1.5 py-0.5 bg-slate-200 dark:bg-zinc-700 text-slate-700 dark:text-zinc-200 rounded font-mono font-bold">F8</kbd> to Save Item
          </div>
          <div class="flex gap-2">
             ...
             <button type="submit" form="stock-form" :disabled="saving" class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-lg shadow-indigo-600/30 hover:bg-indigo-700 transition-all flex items-center gap-2">
