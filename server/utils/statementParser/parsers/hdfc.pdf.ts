...
export async function parseHdfcPdf(input: ParserInput): Promise<RawTransaction[]> {
  const { rows } = await extractPdfRows(input.buffer, input.password, input.rawText);
  const transactions: RawTransaction[] = [];

  const datePattern = /^(\d{2}[-/.]\d{2}[-/.]\d{2,4})/;

  let withdrawalHeaderX = 450;
  let depositHeaderX = 550;
  ...
  for (const row of rows) {
    ...
    if (
      text.startsWith('HDFC BANK') ||
      text.startsWith('Statement of Account') ||
      text.startsWith('Page No') ||
      text.startsWith('Date Narration') ||
      text.startsWith('CLOSING BALANCE') ||
      text.startsWith('STATEMENT SUMMARY')
    ) {
      continue;
    }
