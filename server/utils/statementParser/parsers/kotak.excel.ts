...
export async function parseKotakExcel(input: ParserInput): Promise<RawTransaction[]> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (rowText.includes('description') || rowText.includes('narration') || rowText.includes('particulars')) {
      headerRowIndex = rowNumber;
      values.forEach((cellVal, colIdx) => {
        const str = String(cellVal || '').toLowerCase().trim();
        if (str.includes('date')) colDate = colIdx;
        else if (str.includes('description') || str.includes('narration') || str.includes('particulars')) colNarration = colIdx;
