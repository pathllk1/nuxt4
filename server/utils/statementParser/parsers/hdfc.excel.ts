...
export async function parseHdfcExcel(input: ParserInput): Promise<RawTransaction[]> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (rowText.includes('narration') && (rowText.includes('withdrawal') || rowText.includes('deposit') || rowText.includes('debit'))) {
      headerRowIndex = rowNumber;
      values.forEach((cellVal, colIdx) => {
        const str = String(cellVal || '').toLowerCase().trim();
        if (str.includes('date')) colDate = colIdx;
        else if (str.includes('narration') || str.includes('particulars')) colNarration = colIdx;
