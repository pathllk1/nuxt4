...
export async function parseSbiPdf(input: ParserInput): Promise<RawTransaction[]> {
  ...
  for (const row of rows) {
    ...
    // Amounts are trailing numeric patterns at the right side of the row
    ...
    if (amountItems.length >= 3) {
      debit = parseFloat(amountItems[0].text.replace(/,/g, ''));
      credit = parseFloat(amountItems[1].text.replace(/,/g, ''));
      balance = parseFloat(amountItems[2].text.replace(/,/g, ''));
    } else if (amountItems.length === 2) {
      const amtVal = parseFloat(amountItems[0].text.replace(/,/g, ''));
      balance = parseFloat(amountItems[1].text.replace(/,/g, ''));
