...
export async function parsePnbExcel(input: ParserInput): Promise<RawTransaction[]> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (rowText.includes('remarks') || rowText.includes('txn date')) {
      headerRowIndex = rowNumber;
      values.forEach((cellVal, colIdx) => {
        const str = String(cellVal || '').toLowerCase().trim();
        if (str.includes('date')) colDate = colIdx;
        else if (str.includes('remarks') || str.includes('description') || str.includes('particulars')) colNarration = colIdx;
        else if (str.includes('cheque') || str.includes('chq') || str.includes('ref')) colRef = colIdx;
