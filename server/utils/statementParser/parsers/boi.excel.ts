...
export async function parseBoiExcel(input: ParserInput): Promise<RawTransaction[]> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (rowText.includes('date') && rowText.includes('description') && (rowText.includes('debit') || rowText.includes('credit'))) {
  ...
  for (let i = headerRowIndex + 1; i <= sheet.rowCount; i++) {
    ...
    if (!dateCell || /^total/i.test(dateCell) || /^b\/f/i.test(dateCell)) continue;
