import { defineEventHandler, readBody, createError } from 'h3';
import { getScopedCollection } from '../../../utils/firebase';

export default defineEventHandler(async (event) => {
  const body = await readBody(event);
  if (!body || !body.categoryId || body.limitAmount === undefined || !body.month) {
    throw createError({ statusCode: 400, statusMessage: 'Category, Month, and Limit Amount are required' });
  }

  const id = Date.now() * 1000 + Math.floor(Math.random() * 1000);
  const data = {
    ...body,
    id,
    limitAmount: Number(body.limitAmount),
    createdAt: new Date().toISOString()
  };

  const col = getScopedCollection(event, 'budgets');
  await col.doc(id.toString()).set(data);

  return { success: true, id, data };
});
