...
import { generateProfitLossExcel } from '../../../../utils/accounting/excel-export.utils';
...
export default defineEventHandler(async (event) => {
  ...
  const buffer = await generateProfitLossExcel({
  ...
  setHeader(event, 'Content-Disposition', 'attachment; filename="Profit_and_Loss.xlsx"');
  return buffer;
});
