...
import { generateDrillDownExcel } from '../../../../utils/accounting/excel-export.utils';
...
export default defineEventHandler(async (event) => {
  ...
  const buffer = await generateDrillDownExcel({
  ...
  setHeader(event, 'Content-Disposition', `attachment; filename="DrillDown_${type.replace(/[^a-zA-Z0-9]/g, '_')}.xlsx"`);
  return buffer;
});
