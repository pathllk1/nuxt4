...
import { exportTrialBalanceToPdfBuffer } from '../../../../utils/accounting/pdf-export.utils';
...
export default defineEventHandler(async (event) => {
  ...
  const buffer = await exportTrialBalanceToPdfBuffer({
  ...
  setHeader(event, 'Content-Disposition', 'attachment; filename="Trial_Balance.pdf"');
  return buffer;
});
