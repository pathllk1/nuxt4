...
import { exportLedgerToPdfBuffer } from '../../../utils/accounting/pdf-export.utils';
...
export default defineEventHandler(async (event) => {
  ...
  const buffer = await exportLedgerToPdfBuffer({
  ...
  setHeader(event, 'Content-Disposition', `attachment; filename="Ledger_${accountHead.replace(/[^a-zA-Z0-9]/g, '_')}.pdf"`);
  return buffer;
});
