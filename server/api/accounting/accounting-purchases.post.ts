...
import { getNextBillNumber, getNextVoucherNumber, calcBillTotals, ensureUniqueSupplierBillNo } from '../../utils/accounting/bill-utils';
...
export default defineEventHandler(async (event) => {
  ...
  const executeSave = async (session?: mongoose.ClientSession) => {
    ...
    await LedgerService.postAccountingPurchaseLedger({
      firmId: firmIdObj,
      billId: newBill._id,
      voucherId: String(voucherId),
      billNo,
  ...
  try {
    ...
    return {
      success: true,
      message: 'Accounting purchase bill created successfully',
      data: createdBill,
    };
  } catch (err: any) {
    if (session) {
    ...
    if (
      ...
      console.warn('Retrying accounting purchase bill post without transaction session:', err.message);
      try {
