...
import { generateDayBookPdf } from '../../../utils/accounting/pdf-export.utils';
...
export default defineEventHandler(async (event) => {
  ...
  const buffer = await generateDayBookPdf({
  ...
  setHeader(event, 'Content-Disposition', `attachment; filename="DayBook_${periodText.replace(/[^a-zA-Z0-9]/g, '_')}.pdf"`);
  return buffer;
});
